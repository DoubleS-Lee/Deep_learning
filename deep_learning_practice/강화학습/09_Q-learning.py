# Q-learning
# 다음 상태에 대한 Q값들 중 가장 높은값을 이용해서 업데이트 하는 구조
# off-policy method
# 코드는 이전의 TDAgent를 상속받는 형태로 구현하였음
# 1) 모든 Q(s,a)를 0으로 초기화
# 2) 초기상태 s 관찰
######### 3)~7) 반복 (에피소드 내에서) ########
# 3) Q(s,a)를 활용해서 a 결정    <-  행동정책 mu
# 4) a를 환경에 가한 후 r과 s' 결정
# 5) Q(s,a) <- Q(s,a) + alpha*(reward + gamma*max{a'}Q(s',a') - Q(s,a))    <-   평가정책 pi
# 7) s <- s'
...
5. `lr` : 학습률
'''
qlearning_agent = QLearner(gamma=1.0, lr=1e-1, num_states=env.nS, num_actions=env.nA, epsilon=1.0)
